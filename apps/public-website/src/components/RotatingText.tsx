    ...
    >
      ...
      <AnimatePresence mode={animatePresenceMode} initial={animatePresenceInitial}>
        <motion.span
          ...
          aria-hidden="true"
        >
          {elements.map((wordObj: ElementItem, wordIndex: number, array: ElementItem[]) => {
            const previousCharsCount = array.slice(0, wordIndex).reduce((sum: number, word: ElementItem) => sum + word.characters.length, 0);
            const totalChars = array.reduce((sum: number, word: ElementItem) => sum + word.characters.length, 0);
            return (
